...
class StorageVars():
    ...
    def define_storage_common_vars(self):
        ...
        if _is_relaxed(model):
            model.InputStorage = Var(model.Storage, model.TimePeriods, within=PercentFraction)
            model.OutputStorage = Var(model.Storage, model.TimePeriods, within=PercentFraction)
        ...
        if _is_relaxed(model):
            model.BinStorage_reg = Var(model.BESS_PHS_set, model.TimePeriods, within=PercentFraction)
            model.BinStorage_SP = Var(model.BESS_PHS_set, model.TimePeriods, within=PercentFraction)
            model.BinStorage_NSP = Var(model.BESS_PHS_set, model.TimePeriods, within=PercentFraction)
            model.BinStorage_SUPP = Var(model.BESS_PHS_set, model.TimePeriods, within=PercentFraction)
            # Fix variables to provided values in relaxed case
            for s in model.BESS_PHS_set:
                for t in model.TimePeriods:
